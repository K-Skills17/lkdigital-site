...
/**
 * Five distinct hero treatments for internal pages.
 * Each breaks the "centered text on dark gradient" pattern.
 */
...
export default function PageHero({
  ...
  // ─── EDITORIAL: Huge left-aligned type with vertical rule + page number ───
  if (variant === "editorial") {
    return (
      <section className="relative pt-32 pb-16 md:pt-40 md:pb-24 overflow-hidden border-b border-border">
        <div className="max-w-content mx-auto px-4 sm:px-6">
          <div className="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-8 items-end">
            <div>
              ...
              <h1 className="font-display text-[clamp(2.5rem,6vw,5.5rem)] leading-[1.05] tracking-tight text-foreground">
                {title}
                {titleAccent && (
                  <>
                    <br />
  ...
  // ─── SPLIT: Full-width gradient with left-aligned type ───
  if (variant === "split") {
    return (
      <section className="relative overflow-hidden border-b border-border">
        <div className="relative min-h-[60vh] bg-gradient-to-br from-[#0f0f0f] via-[#1a1510] to-[#2a2318] flex items-end">
          <div className="grain-overlay" />
          <div className="relative z-10 max-w-content mx-auto px-4 sm:px-6 lg:px-16 pt-36 pb-16 md:pt-44 md:pb-20 w-full">
            <div className="grid grid-cols-1 md:grid-cols-[3fr_2fr] gap-10 items-end">
              <div>
                {eyebrow && (
                ...
                <h1 className="font-display text-[clamp(2rem,5vw,4rem)] leading-[1.1] text-white">
                  {title}
                  {titleAccent && (
                    <>
                      <br />
            ...
            <div className="mt-10 h-px bg-gradient-to-r from-accent/50 via-accent/15 to-transparent" />
          </div>
        </div>
      </section>
    );
  ...
  // ─── TYPOGRAPHIC: Full-width massive stacked typography + right CTA column ───
  if (variant === "typographic") {
    const words = title.split(" ");
    return (
      <section className="relative pt-32 pb-12 md:pt-40 md:pb-16 overflow-hidden border-b border-border">
        <div className="max-w-content mx-auto px-4 sm:px-6">
          {eyebrow && (
          ...
          {subtitle && (
            <div className="mt-8 flex items-start gap-6">
              <span className="w-12 h-px bg-accent mt-3 flex-shrink-0 hidden sm:block" />
  ...
  // ─── MINIMAL: Clean white, oversized thin type, horizontal rule ───
  if (variant === "minimal") {
    return (
      <section className="relative pt-32 pb-10 md:pt-40 md:pb-14 overflow-hidden">
        <div className="max-w-content mx-auto px-4 sm:px-6">
          ...
          <h1 className="font-display text-[clamp(2.5rem,5.5vw,4.5rem)] leading-[1.1] font-light text-foreground max-w-4xl">
            {title}
            {titleAccent && (
              <span className="text-accent"> {titleAccent}</span>
            )}
          ...
          {subtitle && (
            <div className="mt-6 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
              ...
              {pageNumber && (
                <span className="font-display text-sm text-muted-foreground tracking-[0.25em] uppercase flex-shrink-0">
                  {pageNumber}
                </span>
              )}
            </div>
  ...
  // ─── DRAMATIC: Dark bg, off-center huge type, accent line across ───
  if (variant === "dramatic") {
    return (
      <section className="relative bg-[#0f0f0f] overflow-hidden">
        <div className="grain-overlay" />
        <div className="relative z-10 max-w-content mx-auto px-4 sm:px-6 pt-36 pb-20 md:pt-44 md:pb-28">
          <div className="grid grid-cols-1 md:grid-cols-[2fr_1fr] gap-10 items-end">
            <div>
              ...
              <h1 className="font-display text-[clamp(2.5rem,6vw,5rem)] leading-[1.05] text-white tracking-tight">
                {title}
                {titleAccent && (
                  <>
                    {" "}
          ...
          <div className="mt-12 h-px bg-gradient-to-r from-accent/60 via-accent/20 to-transparent" />
        </div>
      </section>
    );
  }
